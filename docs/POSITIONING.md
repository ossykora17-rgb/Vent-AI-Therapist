...
| **Human tone** | Enforced, not aspired to: 35 phrases and unasked-for tasks fail the **build**, and a reply carrying one is regenerated before anybody reads it. `voice.ts`, `failsafe.ts` | Scripted decision trees; deliberately consistent | Scripted + LLM hybrid | Free-form; optimises for engagement |
